...
class VentanaPrincipal(QMainWindow):
    def __init__(self):
        ...
        self.combo_poligono.addItems(["TV20", "TV50", "TV51", "TV34", "TV0", "TV52", "TV08", "TV21", "TV33", "General"])
        ...
        self.combo_display.currentIndexChanged.connect(self.habilitar_calculo)  
        ...
        layout_content.setStretch(0, 4) 
        layout_content.setStretch(1, 4) 
    ...
    def obtener_poligono(self):
        ...
        elif poligono_nombre == "TV34":
            return Polygons.TV34()
        elif poligono_nombre == "TV30":
            return Polygons.TV0()
        elif poligono_nombre == "TV52":
            return Polygons.TV52()
        elif poligono_nombre == "TV08":
            return Polygons.TV08()
        elif poligono_nombre == "TV21":
            return Polygons.TV21()
        elif poligono_nombre == "TV33":
            return Polygons.TV33()
        elif poligono_nombre == "General":
            return Polygons.General()
    ...
    def calcular_centro(self):
        ...
        if display_option == "Solo Nodos":
            fig, ax = ox.plot_graph(G, show=False, close=False, node_color='cyan', node_edgecolor='black', bgcolor='black', edge_color='gray', ax=ax)
            ax.set_title("Nodos", color='white')
        elif display_option == "Solo Polígono":
            x_poly, y_poly = zip(*poligono)
            ax.plot(x_poly, y_poly, 'o-', color='blue', label='Polígono')
            ax.set_title("Polígono", color='white')
            ...
            ax.set_title("Nodos con Distancias", color='white')
        else:  # "Centro"
            fig, ax = ox.plot_graph(G, show=False, close=False, edge_color='gray', node_color='cyan', node_edgecolor='black', bgcolor='black', ax=ax)
            ax.scatter(x, y, color='red', s=100, zorder=5, label=metodo_texto)
            x_poly, y_poly = zip(*poligono)
            ax.plot(x_poly, y_poly, 'o-', color='blue', label='Polígono')
            ax.set_title("Centro", color='white')
    ...
    def abrir_en_google_maps(self):
        if self.lat_central is not None and self.lon_central is not None:
            url = f"https://www.google.com/maps?q={self.lat_central},{self.lon_central}"
